...
const examples = [
  {
    ...
    code: `// Fetch repositories from a user
...
export default function Projects() {
  ...
  return (
    <div>
      <h1>{data.stats.total_repos} projects</h1>
    ...
    <section id="examples" className="py-24 relative">
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        <ScrollReveal className="text-center mb-12">
          <span className="text-brand-400 text-sm font-semibold tracking-wider uppercase">{t.examples.badge}</span>
          <h2 className="text-3xl sm:text-4xl font-bold text-white mt-3 mb-4">
            {t.examples.title}<span className="gradient-text">{t.examples.titleHighlight}</span>
          </h2>
          <p className="text-dark-300 max-w-xl mx-auto">{t.examples.subtitle}</p>
        </ScrollReveal>
              ...
              {examples.map((ex) => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm font-medium border transition-all whitespace-nowrap inline-flex items-center gap-2 ${
                    activeExample === ex.id ? "tab-active border-brand-500/30" : "text-dark-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
                  <span className={`font-mono text-xs font-bold ${activeExample === ex.id ? ex.color : ""}`}>{ex.icon}</span>
                  {ex.label}
                </button>
              ))}
            </div>
            ...
            <div className="code-wrapper relative">
              <div className="code-block rounded-xl animate-fade-in">
                <div className="flex items-center justify-between px-5 py-3 border-b border-brand-500/10">
                  ...
                  <button onClick={copyCode} className="copy-btn text-dark-400 hover:text-brand-400 transition-colors text-xs inline-flex items-center gap-1.5">
                    {copied ? (
                      <>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" className="text-green-400"><polyline points="20 6 9 17 4 12" /></svg>
                        {t.examples.copied}
                      </>
                    ) : (
                      <>
                ...
                <pre className="p-5 text-sm overflow-x-auto"><code className="text-dark-200">{current.code}</code></pre>
              </div>
            </div>

            <div className="mt-6 glass-card rounded-xl p-5 flex items-start gap-4">
              <div className="w-10 h-10 rounded-lg bg-brand-500/10 flex items-center justify-center shrink-0">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-brand-400"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
              </div>
              <div>
                <p className="text-sm text-white font-medium mb-1">{t.examples.tip}</p>
                <p className="text-sm text-dark-300">
                  {t.examples.tipText.split("{username}").map((part, i, arr) =>
                    i < arr.length - 1 ? (
                      <span key={i}>{part}<code className="px-1.5 py-0.5 rounded bg-brand-500/10 text-brand-400 text-xs">USERNAME</code></span>
                    ) : part
                  )}
                </p>
              </div>
